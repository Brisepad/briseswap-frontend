...
const Farms: React.FC = () => {
  ...
  const rowData = farmsStakedMemoized.map((farm) => {
    ...
    const lpLabel = farm.lpSymbol && farm.lpSymbol.split(' ')[0].toUpperCase().replace('BRISESWAP', '')
    ...
    <>
      ...
      <Page>
        <ControlContainer>
          ...
          <FilterContainer>
            <LabelWrapper>
              <Text color='textSubtle' textTransform="uppercase">{t('Sort by')}</Text>
              <Select
                options={[
                  {
                    label: t('Hot'),
        ...
        {renderContent()}
        <div ref={loadMoreRef} />
        <StyledImage src="/images/briseswap-bg-logo.svg" alt="Briseswap illustration" width={120} height={103} />
      </Page>
    </>
  )
}
